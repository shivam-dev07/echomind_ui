...
class MainLayout extends StatefulWidget {
  static final GlobalKey<MainLayoutState> globalKey =
...
class MainLayoutState extends State<MainLayout> {
  ...
  Widget build(BuildContext context) {
    final screens = [
      HomeRecordScreen(onMeetingCreated: _handleMeetingCreated),
  ...
  Widget _buildGlassNavBar() {
    return Consumer<RecordingController>(
      builder: (context, recording, child) {
        final hasActiveRecordingOutsideRecordTab =
            recording.isRecording && _currentIndex != 0;

        return AnimatedContainer(
          duration: const Duration(milliseconds: 220),
          margin: EdgeInsets.fromLTRB(
            16,
            hasActiveRecordingOutsideRecordTab ? 8 : 0,
            16,
            12,
          ),
